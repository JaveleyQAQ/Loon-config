...
const ICON = 'https://raw.githubusercontent.com/leiyiyan/resource/main/icons/lhtj.png'; // 去掉尾部空格！

let accounts = loadAccounts();
$.notify = [];

// 兼容 Loon / QX / Surge / Node 的 HTTP 请求
async function http(options) {
  const { url, headers = {}, body, method = 'GET' } = options;
  const isPost = method.toUpperCase() === 'POST' || (options.type && options.type.toLowerCase() === 'post');
  ...
  try {
    if (typeof $httpClient !== 'undefined') {
      ...
      return new Promise((resolve) => {
        ...
        $httpClient[isPost ? 'post' : 'get'](opts, (err, resp, data) => {
          if (err) {
            resolve({ __error: true, message: err });
          } else {
            try {
              data = JSON.parse(data);
            } catch (e) {}
            resolve({ data, status: resp?.status || 200 });
          }
        });
      });
      ...
      let opts = { url: url.trim(), headers };
      if (isPost) opts.body = typeof body === 'string' ? body : JSON.stringify(body);
      const resp = await $task.fetch({ method, ...opts });
      let data;
...
async function doSignin(acc) {
  const url = 'https://kohcamp.qq.com/operation/action/newsignin'; // 修复空格
  const headers = {
    Host: 'kohcamp.qq.com',
    Referer: 'https://camp.qq.com/', // 修复空格
    token: acc.token || '',
    userId: acc.userId || '',
  ...
  const roleId = acc.roleId || acc.campRoleId || '';
  const body = {
    cSystem: 'ios',
    h5Get: 1,
    gameId: '20001',
  ...
  const res = await http({ url, method: 'POST', headers, body });

  let note = '';
  if (res.__error) {
    note = '❌ 请求异常: ' + res.message;
  } else {
    const json = res.data || {};
    const rc = json.returnCode ?? json.code ?? undefined;
    if (rc === 0) {
      const txt = json.data?.text || JSON.stringify(json.data || json);
      note = '✅ 签到成功: ' + txt;
    } else if (rc === -105203 || /请勿重复签到/.test(json.returnMsg || '')) {
      note = 'ℹ️ 今日已签到';
    } else {
      note = '❌ 签到失败: ' + (json.returnMsg || JSON.stringify(json));
    }
  }
...
function extractFromRequest() {
  ...
  for (let key in rawHeader) {
    lowerHeader[key.toLowerCase()] = rawHeader[key];
  }

  let bodyObj = {};
  try {
    bodyObj = $request.body ? JSON.parse($request.body) : {};
  } catch (e) {}
  ...
  return { header: lowerHeader, bodyObj, q };
}
...
async function captureCookie() {
  ...
  if (!newData.token && !newData.userId && !newData.campRoleId) {
    $.msg('kohcamp 获取信息', '', '⚠️ 未检测到有效凭证，未保存', { icon: ICON });
    return;
  }

  const idx = accounts.findIndex(a => a.token && a.token === newData.token);
  if (idx !== -1) {
    accounts[idx] = { ...accounts[idx], ...newData };
  } else {
    accounts.push(newData);
  }

  saveAccounts(accounts);
  $.msg('🎉 获取凭证成功', '', `已保存 ${newData.userName} (${newData.userId || 'no-id'})`, { icon: ICON });
}
...
async function nodeNotifyAll() {
  ...
  try {
    ...
    console.log('⚠️ sendNotify 加载失败:', e.message);
  }
}
...
async function main() {
  ...
  if (!accounts || accounts.length === 0) {
    $.msg('kohcamp 签到', '', '❌ 未找到账户，请先访问营地签到页获取 Cookie', { icon: ICON });
    return;
  }
...
!(async () => {
  try {
    ...
    $.msg($.name, '⛔️ 脚本运行出错', e.message || String(e), { icon: ICON });
  } finally {
    $.done(); // 不传参数更安全
  }
})();
...
function saveAccounts(obj) {
  ...
  if (typeof $persistentStore !== 'undefined') return $persistentStore.write(s, ckName);
  if (typeof $prefs !== 'undefined') return $prefs.setValueForKey(s, ckName);
  if (typeof $storage !== 'undefined') return $storage.setItem(ckName, s);
  if (typeof $task !== 'undefined') return $task.set(ckName, s);
  if (typeof localStorage !== 'undefined') return localStorage.setItem(ckName, s);
  // Node: 可通过 process.env 手动注入，不自动写文件
}

function loadAccounts() {
  let val;
  if (typeof $persistentStore !== 'undefined') val = $persistentStore.read(ckName);
  else if (typeof $prefs !== 'undefined') val = $prefs.valueForKey(ckName);
  else if (typeof $storage !== 'undefined') val = $storage.getItem(ckName);
  else if (typeof $task !== 'undefined') val = $task.get(ckName);
  else if (typeof localStorage !== 'undefined') val = localStorage.getItem(ckName);
  else if (typeof process !== 'undefined' && process.env[ckName]) val = process.env[ckName];

  try {
    return val ? JSON.parse(val) : [];
  } catch (e) {
    console.log('⚠️ 账户数据解析失败，返回空数组');
    return [];
  }
}
